...
df_diab_binary = pd.read_csv(
    "C:/pix/ml/mlproject_diabetes/data/diabetes_binary_health_indicators_BRFSS2015.csv"
    if os.uname().sysname != "Linux"
    else "/home/jitghosh/work/pix/ml/mlproject_diabetes/data/diabetes_binary_health_indicators_BRFSS2015.csv"
)
# %%
# feature selection
...
cat_features_idx = [idx for idx,name in enumerate(train_X.columns) if name in cat_features]
...
def resample(train_X,train_y,cat_features_idx):
    ...
    return smoteenn.fit_resample(train_X,train_y)
#%%
...
for train_idx,valid_idx in skfold.split(train_X,train_y):
    resampled_train_X,resampled_train_y = resample(train_X=train_X.iloc[train_idx,:],train_y=train_y[train_idx])
    valid_X,valid_y = train_X.iloc[valid_idx,:],train_y.iloc[valid_idx]

    ds_train_X = lgbm.Dataset(
        resampled_train_X,
        label=resampled_train_y,
        feature_name=all_features,
        categorical_feature=cat_features,
        weight=compute_sample_weight(class_weight="balanced", y=resampled_train_y),
    )
